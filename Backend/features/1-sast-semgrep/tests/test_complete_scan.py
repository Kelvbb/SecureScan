...
try:
    ...
    tool_execs = session.query(ToolExecution).filter(ToolExecution.scan_id == scan.id).all()
    ...
    session.delete(scan)  # Cascade delete automatique des tool_executions et vulnerabilities
    session.delete(user)
    session.commit()
