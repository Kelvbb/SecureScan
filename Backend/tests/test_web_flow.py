...
async def test_web_flow(repo_url: str):
    ...
    try:
        ...
        if not user:
            ...
            print("   Créez d'abord un utilisateur via l'interface web ou le script de test")
            return
        ...
        print(f"✅ Technologies détectées: {', '.join(detected_techs) if detected_techs else 'Aucune'}")
        ...
        for tool_exec in tool_execs:
            ...
            if tool_exec.finished_at and tool_exec.started_at:
                duration = (tool_exec.finished_at - tool_exec.started_at).total_seconds()
