...
""")
        ...
        try:
            subprocess.run(
                ["git", "init"], cwd=str(test_dir), capture_output=True, timeout=10
            )
            subprocess.run(
                ["git", "add", "."], cwd=str(test_dir), capture_output=True, timeout=10
            )
            subprocess.run(
        ...
        try:
            cmd = ["trufflehog", "--json", "--regex", str(test_dir)]
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if installed_tools.get("semgrep"):
                ...
                if result.get("status") == "success":
                    ...
                    print(f"  Results: {len(result.get('results', []))} issue(s)")
            ...
            if installed_tools.get("trufflehog"):
                ...
                if result.get("status") == "success":
                    print(f"  ✓ Result structure valid")
                    print(f"  Secrets: {len(result.get('secrets', []))} secret(s)")
                elif result.get("status") == "error":
                    print(f"  ✗ Error: {result.get('error')}")
                elif result.get("status") == "skipped":
                    print(f"  ⊘ Skipped: {result.get('reason')}")
