...
async def test_analyze_repo():
    ...
    try:
        ...
        code_files = (
            list(project_path_obj.rglob("*.py"))
            + list(project_path_obj.rglob("*.ts"))
            + list(project_path_obj.rglob("*.tsx"))
            + list(project_path_obj.rglob("*.js"))
            + list(project_path_obj.rglob("*.jsx"))
        )
        ...
        if vulns_in_db:
            ...
            owasp_counts = Counter(
                v.owasp_category_id for v in vulns_in_db if v.owasp_category_id
            )
            if owasp_counts:
