...
from app.core.classification import map_rule_to_owasp, map_severity_to_owasp_default, normalize_severity
...
class ScanOrchestrator:
    ...
    async def _execute_scan(self, scan_id: UUID, project_path: str) -> dict:
        ...
        try:
            ...
            for (tool_name, _), result in zip(self.services, results):
                ...
                if result is None:
                    logger.error(f"Tool {tool_name} returned None instead of a result dict")
                    tool_exec = ToolExecution(
                        scan_id=scan_id,
                        status="error",
                ...
                analyzed_files = self._extract_analyzed_files(tool_name, result, project_path)
                ...
                logger.info(f"Tool {tool_name} parsed {len(vulns)} vulnerabilities from result")
                
                # Logger les détails si aucune vulnérabilité n'est trouvée
                if len(vulns) == 0 and result.get("status") == "success":
                    logger.warning(f"Tool {tool_name} returned success but no vulnerabilities found")
                    logger.warning(f"Result keys: {list(result.keys())}")
                    if "results" in result:
                        logger.warning(f"Number of results in raw_output: {len(result.get('results', []))}")
                    if "raw_output" in result and isinstance(result["raw_output"], dict):
                        raw_results = result["raw_output"].get("results", [])
                        logger.warning(f"Number of results in raw_output.results: {len(raw_results)}")
                        if raw_results:
                            logger.info(f"First result example: {json.dumps(raw_results[0], indent=2)[:500]}")
                
                for vuln_data in vulns:
                    try:
                        # Normaliser la sévérité
                        normalized_severity = normalize_severity(vuln_data.get("severity", "medium"))
                        ...
                        if not owasp_category_id:
                            ...
                            owasp_category_id = map_severity_to_owasp_default(normalized_severity)
                        
                        logger.debug(f"Mapping OWASP pour {rule_id} ({tool_name}): {owasp_category_id}")
                        ...
                        logger.debug(f"Ajouté vulnérabilité: {vuln_data.get('title', 'Unknown')[:50]} dans {vuln_data.get('file_path', 'Unknown')} [OWASP: {owasp_category_id}]")
                    except Exception as e:
                        logger.error(f"Erreur lors de l'ajout d'une vulnérabilité en base: {e}")
                        logger.error(f"Données de la vulnérabilité: {json.dumps(vuln_data, indent=2)[:500]}")
                        import traceback
                ...
                try:
                    self.db.flush()
                    logger.info(f"Flush réussi: {len(vulns)} vulnérabilités de {tool_name} ajoutées en base")
                    
                    # Commit immédiatement après chaque outil pour garantir la persistance
                    self.db.commit()
                    logger.info(f"Commit réussi: {len(vulns)} vulnérabilités de {tool_name} sauvegardées en base de données")
                    
                    # Attendre un peu pour s'assurer que le commit est bien propagé
                    time.sleep(0.5)
                    
                    # Vérifier que les vulnérabilités sont bien en base
                    count_in_db = self.db.query(Vulnerability).filter(
                        Vulnerability.scan_id == scan_id,
                        Vulnerability.tool_execution_id == tool_exec.id
                    ).count()
                    logger.info(f"Vérification: {count_in_db} vulnérabilités de {tool_name} trouvées en base après commit")
                    
                    if count_in_db != len(vulns):
                        logger.warning(f"ATTENTION: {len(vulns)} vulnérabilités parsées mais seulement {count_in_db} en base!")
                        # Réessayer le commit si nécessaire
                        if count_in_db == 0 and len(vulns) > 0:
                            logger.warning(f"Tentative de nouveau commit pour {tool_name}...")
                            self.db.commit()
                            time.sleep(0.5)
                            count_in_db = self.db.query(Vulnerability).filter(
                                Vulnerability.scan_id == scan_id,
                            ...
                            logger.info(f"Après nouveau commit: {count_in_db} vulnérabilités en base")
                except Exception as e:
                ...
                logger.info(f"Tool {tool_name} found {len(vulns)} vulnerabilities and analyzed {len(analyzed_files)} files")
            ...
            total_in_db = self.db.query(Vulnerability).filter(
                Vulnerability.scan_id == scan_id
            ).count()
            logger.info(f"Total de vulnérabilités en base pour le scan {scan_id}: {total_in_db}")
            
            if total_in_db == 0 and len(all_vulns) > 0:
                logger.error(f"ERREUR CRITIQUE: {len(all_vulns)} vulnérabilités parsées mais aucune en base!")
                ...
                for vuln_data in all_vulns[:10]:  # Limiter à 10 pour le test
                    try:
                        ...
                        tool_exec = self.db.query(ToolExecution).filter(
                        ...
                        if tool_exec:
                            ...
                            tool_for_mapping = tool_name if tool_name in ["semgrep", "bandit", "eslint"] else "semgrep"
                            owasp_category_id = map_rule_to_owasp(rule_id, tool_for_mapping)
                            if not owasp_category_id:
                                owasp_category_id = map_severity_to_owasp_default(normalized_severity)
            ...
            try:
                self.db.commit()
                logger.info(f"Scan {scan_id} marqué comme terminé avec {total_in_db} vulnérabilités en base")
                
                # Attendre un peu et vérifier une dernière fois
                time.sleep(1)
                final_check = self.db.query(Vulnerability).filter(
                    Vulnerability.scan_id == scan_id
                ).count()
                logger.info(f"Vérification finale: {final_check} vulnérabilités en base après commit final")
                
                if final_check != total_in_db:
                    logger.warning(f"Différence détectée: {total_in_db} avant commit, {final_check} après")
            except Exception as e:
    ...
    def _list_all_code_files(self, project_path: str) -> set[str]:
        ...
        code_extensions = {
            '.py', '.js', '.ts', '.jsx', '.tsx', '.php', '.java', '.go', '.rb',
            '.swift', '.kt', '.scala', '.c', '.cpp', '.cs', '.rs', '.sh', '.bash',
    ...
    def _extract_analyzed_files(self, tool_name: str, result: dict, project_path: str | None = None) -> list[str]:
        """
            ...
            for finding in result.get("results", []):
                ...
                if file_path:
                    ...
                    if project_path and file_path.startswith(project_path):
                        try:
                            file_path = str(Path(file_path).relative_to(Path(project_path)))
                        except ValueError:
                            pass
                    files.add(file_path)
                ...
                for file_path in analyzed_files:
                    if isinstance(file_path, str):
                        ...
                        if project_path and file_path_str.startswith(project_path):
                            try:
                                file_path_str = str(Path(file_path_str).relative_to(Path(project_path)))
                            except ValueError:
                                pass
    ...
    def _parse_vulnerabilities(self, tool_name: str, result: dict) -> list[dict]:
        ...
        """
        logger.info(f"Parsing vulnérabilités pour {tool_name}, status: {result.get('status')}")
