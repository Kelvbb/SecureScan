...
class ESLintService:
    ...
    async def run(project_path: str) -> dict:
        ...
        try:
            ...
            node_modules_security = project_path_obj / "node_modules" / "eslint-plugin-security"
            if not node_modules_security.exists():
                # Chercher dans les sous-dossiers
                for subdir in ["frontend", "Frontend", "src", "app"]:
                    candidate = project_path_obj / subdir / "node_modules" / "eslint-plugin-security"
                    if candidate.exists():
                        node_modules_security = candidate
                        break
                else:
                    # Le plugin n'est pas installé, essayer de l'installer
                    logger.info("Plugin eslint-plugin-security non trouvé, tentative d'installation...")
                    try:
                        # Installer le plugin dans le projet
                        install_cmd = ["npm", "install", "--no-save", "eslint-plugin-security"]
                        ...
                        await asyncio.wait_for(install_process.communicate(), timeout=60.0)
                        logger.info("Plugin eslint-plugin-security installé avec succès")
                    except Exception as e:
                        logger.warning(f"Impossible d'installer eslint-plugin-security: {e}")
            ...
            try:
                ...
                if node_modules_security.exists():
                    ...
                    logger.warning("Utilisation d'ESLint sans plugin security (règles de base)")
                    config_content = {
                        "parserOptions": {
                            "ecmaVersion": 2020,
                            "sourceType": "module"
            ...
            valid_keys = ["extends", "plugins", "parserOptions", "rules", "env", "parser", "globals"]
            clean_config = {
                k: v for k, v in config_content.items() 
                if k in valid_keys
            }
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as config_file:
                ...
                logger.debug(f"Contenu de la config: {json.dumps(clean_config, indent=2)}")
            ...
            if "extends" in config_content and "plugin:security/recommended" in config_content.get("extends", []):
            ...
            try:
                ...
                process = await asyncio.create_subprocess_exec(
                    *cmd,
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE,
                    cwd=str(project_path_obj.parent if package_json.parent != project_path_obj else project_path_obj),
                )
                ...
                try:
                    ...
                    logger.info(f"ESLint: {len(all_results)} résultats trouvés dans {len(analyzed_files)} fichiers")
                ...
                return {
                    ...
                    "results": all_results,
                    "errors": [],
                    "stats": {"files_analyzed": len(analyzed_files), "issues_found": len(all_results)},
    ...
    def parse_vulnerabilities(eslint_results: dict) -> list[dict]:
        ...
        if eslint_results.get("status") != "success":
            logger.warning(f"ESLint status n'est pas 'success': {eslint_results.get('status')}")
