...
class TechnologyDetector:
    ...
    def detect(project_path: str) -> Dict[str, bool]:
            ...
            for indicator in files:
                if indicator.startswith("."):
                    ...
                    if TechnologyDetector._has_extension(project, indicator):
                        ...
                        logger.info(
                            f"Technologie {tech} détectée via extension {indicator}"
                        )
                        break
                else:
                    # Fichier ou dossier
                    if TechnologyDetector._find_file(project, indicator):
                        technologies[tech] = True
                        logger.info(
                            f"Technologie {tech} détectée via fichier {indicator}"
                        )
                        break
        ...
        if technologies["typescript"]:
            if not TechnologyDetector._has_extension(project, ".ts"):
                ...
                technologies["typescript"] = TechnologyDetector._has_extension(
                    project, ".ts", check_subdirs=True
                )
        ...
        if TechnologyDetector._find_file(project, "package.json"):
            ...
            if not technologies["javascript"]:
                technologies["javascript"] = TechnologyDetector._has_extension(
                    project, ".js"
                )

        # Détection PHP : vérifier les fichiers .php
        if TechnologyDetector._find_file(
            project, "composer.json"
        ) or TechnologyDetector._has_extension(project, ".php"):
        ...
        if detected:
            ...
            logger.warning(
                "Aucune technologie détectée, utilisation de la détection par défaut"
            )
            return TechnologyDetector._default_detection()
    ...
    @staticmethod
    def _has_extension(
        project: Path, extension: str, check_subdirs: bool = True
    ) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Erreur lors de la vérification de l'extension {extension}: {e}"
            )
    ...
    def _default_detection() -> Dict[str, bool]:
        ...
        logger.warning(
            "Utilisation de la détection par défaut (toutes technologies activées)"
        )
        return {
            "python": True,
            "javascript": True,
