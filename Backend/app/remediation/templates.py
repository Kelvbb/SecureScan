...
def _fix_sql_injection(line: str) -> FixResult:
    ...
    fstring_pattern = re.compile(
        r'(cursor\.execute\()(f["\'])(.*?)(["\'])\)', re.DOTALL
    )
    percent_pattern = re.compile(
        r'(cursor\.execute\()(["\'])(.*?)["\'](\s*%\s*)(\w+)\)', re.DOTALL
    )
    format_pattern = re.compile(
        r'(cursor\.execute\()(["\'])(.*?)["\']\.format\(([^)]+)\)\)', re.DOTALL
    )
    ...
    if m := fstring_pattern.search(stripped):
        ...
        explanation = (
            "Formatage % remplacé par un tuple de paramètres liés. [OWASP A05]"
        )
        ...
        explanation = (
            ".format() dans execute() remplacé par des paramètres liés. [OWASP A05]"
        )
...
def _fix_xss(line: str) -> FixResult:
    ...
    if m := js_inner.search(stripped):
        ...
        fixed = stripped.replace(
            m.group(0), f"{element}.innerHTML = DOMPurify.sanitize({value});"
        )
        return FixResult(
            original_line=line,
            fixed_line=" " * indent + fixed.lstrip(),
