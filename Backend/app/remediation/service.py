...
@dataclass
class ApplyResult:
    applied:  list[uuid.UUID] = field(default_factory=list)  # IDs appliqués avec succès
    skipped:  list[uuid.UUID] = field(default_factory=list)  # IDs ignorés (erreur)
    errors:   dict[str, str]  = field(default_factory=dict)  # str(id) → message d'erreur
...
class RemediationService:
        ...
        for rel_path, pairs in by_file.items():
            ...
            for sf, vuln in sorted_pairs:
                ...
                if idx < 0 or idx >= len(lines):
                    msg = f"Ligne {vuln.line_number} hors limites ({len(lines)} lignes dans {rel_path})."
                ...
                if not sf.patch_diff:
                    # patch_diff absent : regénérer depuis le template
                    try:
                        fix = generate_fix(VulnerabilityType(vuln.vuln_type), lines[idx])
                ...
                logger.info("Correction appliquée — fix_id=%s fichier=%s ligne=%d",
                            sf.id, rel_path, vuln.line_number)
    ...
    def _generate_and_persist(self, vuln: Vulnerability) -> FixProposalDTO | None:
        ...
        try:
            ...
            fix: FixResult = generate_fix(VulnerabilityType(vuln.vuln_type), original_line)
        ...
        return FixProposalDTO(
            ...
            patch_diff       = patch_diff,
            description      = fix.explanation,
            owasp_category   = fix.owasp_category,
            fix_type         = vuln.vuln_type,
            auto_applicable  = True,
        )

    def _suggested_fix_to_dto(self, sf: SuggestedFix, vuln: Vulnerability) -> FixProposalDTO | None:
        ...
        fixed_line = (
            ...
            if sf.patch_diff else original_line
        )
        return FixProposalDTO(
            suggested_fix_id = sf.id,
            vuln_id          = vuln.id,
            ...
            owasp_category   = "",  # non stocké dans SuggestedFix
    ...
    def _resolve_path(self, relative_path: str) -> Path:
        ...
        if not resolved.is_relative_to(self.project_root.resolve()):
            raise ValueError(f"Path traversal détecté : '{relative_path}' sort de project_root.")
        return resolved
    ...
    def _read_line(self, relative_path: str, line_number: int) -> str:
        ...
        idx      = line_number - 1
        if idx < 0 or idx >= len(lines):
            raise OSError(f"Ligne {line_number} introuvable dans {abs_path} ({len(lines)} lignes).")
        return lines[idx]
    ...
    @staticmethod
    def _make_unified_diff(file_path: str, line_number: int, original: str, fixed: str) -> str:
        import difflib
    ...
    def _extract_fixed_lines_from_diff(patch_diff: str) -> str:
        ...
        fixed_lines = [
            line[1:] for line in patch_diff.splitlines(keepends=True)
