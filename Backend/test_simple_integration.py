...
try:
    r = requests.post(
        ...
        json={"user_id": user_id, "repository_url": "file:///tmp/test_project", "language": "python"},
        timeout=10
    )
    if r.status_code == 201:
        ...
        if r.status_code == 202:
            ...
            for i in range(45):  # Max 90 seconds
                r = requests.get(f"http://127.0.0.1:8000/api/scans/{scan_id}", timeout=10)
            ...
            # 5. Check results
            session.expire_all()
            tools = session.query(ToolExecution).filter(ToolExecution.scan_id == scan_id).all()
            vulns = session.query(Vulnerability).filter(Vulnerability.scan_id == scan_id).all()
            
            print(f"\n[5] Results:")
            print(f"     Tools executed: {len(tools)}")
            for t in tools:
                tool_name = t.raw_output.get('tool', '?') if t.raw_output else '?'
                status_str = f"✓ {t.status}" if t.status == "success" else f"✗ {t.status}"
